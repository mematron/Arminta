...
*   **Symbol Weights**: Every term she uses: action names, emotion labels, mode names, situation types, hypothesis relation types: accumulates a reward-weighted co-occurrence score drawn from her episodic log. The weight is not assigned; it accretes from use. `calm` and `set_ac_max_perf` have different weights because they have appeared in different reward contexts across 200,000 steps. The symbol set grows as new terms enter the episodic record.
...
*   **Statement Formation**: During `DREAM` and `SELF_ASSESS` cycles she composes statements she has never made before, from grammar she observed herself. Her first formed statement: `io_bound calls OPTIMIZE`. A more recent one: `curious during DREAM / compile calls OBSERVE`. Between them, a grammar has assembled itself.
